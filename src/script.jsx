...
import { Package, LayoutGrid, Sparkles, Search, Filter, Trophy, Info, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react';
                ...
                <div className="absolute inset-0 w-full h-full backface-hidden rounded-2xl overflow-hidden bg-zinc-900 border-4 border-zinc-800 shadow-2xl">
                    ...
                    <div className="p-4 flex flex-col gap-2 h-full">
                        ...
                        <div className="mt-auto flex justify-between items-end pb-8">
                            ...
                            {isNew && <span className="px-2 py-0.5 bg-yellow-500 text-black text-[10px] font-black rounded uppercase animate-pulse">New!</span>}
                        </div>
                    </div>
                </div>
                {/* Back */}
                ...
                {CARD_POOL.map((card) => {
                    ...
                    return (
                        <div key={card.id} className={`relative aspect-[2/3] rounded-xl overflow-hidden transition-all duration-500 ${isOwned ? "cursor-pointer hover:scale-105" : "grayscale opacity-40 brightness-50"}`} onClick={() => isOwned && onSelectCard(card)}>
                            <img src={card.imageUrl} className="w-full h-full object-cover" />
                            {isOwned && <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent p-3 flex flex-col justify-end"><span className="text-[10px] font-black uppercase text-zinc-400">x{count}</span><span className="text-xs font-bold text-white truncate">{card.name}</span></div>}
                        </div>
                    );
                })}
            </div>
